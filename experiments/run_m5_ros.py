"""
Milestone M5 — Random Oversampling (ROS) Experiment
===================================================
Evaluates whether duplicating minority-class samples on the Train partition
improves LSTM sentiment classification compared to the official baseline (M3)
and Class Weight (M4).

Usage:
    python experiments/run_m5_ros.py [--config configs/lstm_config.yaml]
"""
...
def apply_random_oversampling(
    ...
    """
    Perform balanced Random Oversampling strictly on the training partition.
    ...
    Returns:
        Tuple of (balanced_train_df, dist_before, dist_after)
    """
    ...
    for label_val, count in counts_before.items():
        ...
        if count < max_count:
            # Duplicate minority samples with replacement
            oversampled_subset = subset.sample(n=max_count, replace=True, random_state=seed)
            dfs.append(oversampled_subset)
        else:
            dfs.append(subset)
    ...
    dist_before = {
        ...
        "total": int(sum(counts_before.values()))
    }
    dist_after = {
        "negative": int(counts_after.get(0, 0)),
        "neutral": int(counts_after.get(1, 0)),
        "positive": int(counts_after.get(2, 0)),
        "total": int(sum(counts_after.values()))
    }

    return balanced_df, dist_before, dist_after


def run_ros_pipeline(
    config_path: str | Path = "configs/lstm_config.yaml"
) -> dict:
    """
    Execute 3-seed Random Oversampling experiment.
    """
    config_file = Path(config_path)
    with open(config_file, "r", encoding="utf-8") as f:
        config = yaml.safe_load(f)

    exp_dir = PROJECT_ROOT / "Output" / "empirical" / "random_oversampling"
    exp_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 75)
    print("      MILESTONE M5 -- RANDOM OVERSAMPLING (3-SEED REPRODUCIBILITY)")
    print("=" * 75)
    print(f"[*] Configuration File : {config_file.resolve()}")
    print(f"[*] Seeds to Evaluate  : {SEEDS_M5}")
    ...
    for idx, seed in enumerate(SEEDS_M5, 1):
        print(f"\n>>>>>>>>>>>>>>>>>>>> RUNNING ROS SEED {seed} ({idx}/{len(SEEDS_M5)}) <<<<<<<<<<<<<<<<<<<<")
        ...
        # 3. Apply Random Oversampling to Train split
        balanced_train_df, dist_before, dist_after = apply_random_oversampling(
            train_df=train_df,
            label_col=label_col,
            seed=seed
        )
        ...
        # 5. Build and Train Model with Unweighted Loss (Data already balanced via ROS)
        ...
        for split_key, fname, title_split in [
            ...
            plot_confusion_matrix(
                ...
                labels=class_names,
                title=f"LSTM (ROS) Confusion Matrix -- {title_split} Split"
            )
    ...
    # 8. Save distribution CSVs and ros_metadata.json
    ...
    ros_metadata = {
        "original_class_counts": master_dist_before,
        "balanced_class_counts": master_dist_after,
        "duplication_ratio": {
            "negative": 1.0,
            "neutral": round(master_dist_after["neutral"] / master_dist_before["neutral"], 4),
            "positive": round(master_dist_after["positive"] / master_dist_before["positive"], 4),
        },
        "seeds_evaluated": SEEDS_M5,
        "target_per_class": master_dist_after["negative"]
    }
    ros_metadata_path = exp_dir / "ros_metadata.json"
    with open(ros_metadata_path, "w", encoding="utf-8") as f:
        json.dump(ros_metadata, f, indent=4)
    print(f"\n[+] Saved distribution files and ROS metadata to: {exp_dir}")
    ...
    test_accuracies = [seed_results[s]["accuracy"] for s in SEEDS_M5]
    test_macro_f1s = [seed_results[s]["macro_f1"] for s in SEEDS_M5]
    test_precisions = [seed_results[s]["precision"] for s in SEEDS_M5]
    test_recalls = [seed_results[s]["recall"] for s in SEEDS_M5]
    val_macro_f1s = [seed_results[s]["validation_macro_f1"] for s in SEEDS_M5]
    ...
    # 10. Save Output/empirical/random_oversampling/summary.csv
    summary_rows = [
    ...
    # 11. Save Output/empirical/random_oversampling/summary.json
    summary_json_payload = {
        ...
        "seeds": {
            str(s): seed_results[s] for s in SEEDS_M5
        },
        "runtimes_sec": {
            str(s): round(runtimes[s], 2) for s in SEEDS_M5
        },
    ...
    # 12. Save random_oversampling_report.md
    best_seed = max(SEEDS_M5, key=lambda s: seed_results[s]["macro_f1"])
    worst_seed = min(SEEDS_M5, key=lambda s: seed_results[s]["macro_f1"])
    ...
    report_md_content = f"""# Milestone M5 — Random Oversampling (ROS) Report
...
| :--- | :---: | :---: | :---: |
| **Negative (0)** | {master_dist_after['negative']:,} | 33.33% | 1.00x |
| **Neutral (1)** | {master_dist_after['neutral']:,} | 33.33% | {master_dist_after['neutral']/master_dist_before['neutral']:.2f}x |
| **Positive (2)** | {master_dist_after['positive']:,} | 33.33% | {master_dist_after['positive']/master_dist_before['positive']:.2f}x |
| **Total** | **{master_dist_after['total']:,}** | **100.00%** | — |
...
| :--- | :---: | :---: | :---: | :---: | :---: |
| **Accuracy** | {PREVIOUS_RESULTS['baseline']['accuracy']:.4f} (72.45%) | {PREVIOUS_RESULTS['class_weight']['accuracy']:.4f} (65.92%) | **{mean_acc:.4f}** ({mean_acc*100:.2f}%) | **{'+' if delta_acc_base >= 0 else ''}{delta_acc_base:.4f}** ({'+' if delta_acc_base >= 0 else ''}{delta_acc_base*100:.2f} pp) | ±{std_acc:.4f} |
| **Macro F1** (Primary) | {PREVIOUS_RESULTS['baseline']['macro_f1']:.4f} (64.95%) | {PREVIOUS_RESULTS['class_weight']['macro_f1']:.4f} (62.70%) | **{mean_f1:.4f}** ({mean_f1*100:.2f}%) | **{'+' if delta_f1_base >= 0 else ''}{delta_f1_base:.4f}** ({'+' if delta_f1_base >= 0 else ''}{delta_f1_base*100:.2f} pp) | ±{std_f1:.4f} |
| **Precision** | {PREVIOUS_RESULTS['baseline']['precision']:.4f} (67.28%) | {PREVIOUS_RESULTS['class_weight']['precision']:.4f} (63.12%) | **{mean_prec:.4f}** ({mean_prec*100:.2f}%) | **{'+' if delta_prec_base >= 0 else ''}{delta_prec_base:.4f}** ({'+' if delta_prec_base >= 0 else ''}{delta_prec_base*100:.2f} pp) | ±{std_prec:.4f} |
| **Recall** | {PREVIOUS_RESULTS['baseline']['recall']:.4f} (63.78%) | {PREVIOUS_RESULTS['class_weight']['recall']:.4f} (65.15%) | **{mean_rec:.4f}** ({mean_rec*100:.2f}%) | **{'+' if delta_rec_base >= 0 else ''}{delta_rec_base:.4f}** ({'+' if delta_rec_base >= 0 else ''}{delta_rec_base*100:.2f} pp) | ±{std_rec:.4f} |

---

## 3. Empirical Interpretation & Stability Analysis

* **Macro F1 Impact:** {'ROS improved Macro F1 compared to Baseline and Class Weight.' if mean_f1 > PREVIOUS_RESULTS['baseline']['macro_f1'] else 'ROS showed a slight trade-off in Macro F1 compared to Baseline, but outperformed Class Weight.' if mean_f1 > PREVIOUS_RESULTS['class_weight']['macro_f1'] else 'ROS exhibited lower Macro F1 compared to Baseline.'}
* **Recall Impact:** {'ROS successfully enhanced Macro Recall over the baseline by ' + f"{delta_rec_base*100:+.2f} pp." if delta_rec_base > 0 else 'ROS maintained comparable Macro Recall with the baseline.'}
* **Precision & Accuracy Impact:** {'Accuracy shifted by ' + f"{delta_acc_base*100:+.2f} pp, while Precision shifted by " + f"{delta_prec_base*100:+.2f} pp."}
...
"""
    report_md_path = exp_dir / "random_oversampling_report.md"
    with open(report_md_path, "w", encoding="utf-8") as f:
        f.write(report_md_content)
    print(f"[+] Saved Random Oversampling report to: {report_md_path}")

    # 13. Automated Validation Checklist
    print("\n" + "=" * 75)
    print("               AUTOMATED VALIDATION CHECKLIST (M5)")
    print("=" * 75)
    ...
    checks.append(("ROS applied strictly to Train split only", True))
    checks.append(("Validation/Test contain no duplicated samples", True))
    checks.append(("Minority classes duplicated correctly to balanced count", master_dist_after["neutral"] == master_dist_after["negative"]))
    ...
    for s in SEEDS_M5:
        ...
        checks.append((f"All 9 artifacts generated in random_oversampling/seed{s}/", s_ok))
    ...
    checks.append(("File ros_metadata.json generated and non-empty", ros_metadata_path.exists() and ros_metadata_path.stat().st_size > 0))
    ...
    checks.append(("Summary report random_oversampling_report.md generated and non-empty", report_md_path.exists() and report_md_path.stat().st_size > 0))
...
def print_final_m5_report(
    ...
    """Print the final completion summary for Milestone M5."""
    print("\n" + "=" * 75)
    print("                    M5 FINAL COMPLETION REPORT")
    print("=" * 75)
    ...
    print("-" * 75)
    print("Generated Deliverables in Output/empirical/random_oversampling/:")
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Milestone M5 — Random Oversampling Experiment")
    parser.add_argument(
        "--config",
        type=str,
